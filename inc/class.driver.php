...
class driver {
  ...
  function authenticate($did, $phone_hash) {
    ...
    if ($row) {
      ...
      $driver_id = $row['id'];
      apilog("인증키생성 $driver_id");

      $this->update_appkey($phone_hash, $token);
      return $token;

    } else {
      apilog("인증키생성 실패 hash:$phone_hash");
      return false; // 실패
    }
  }
